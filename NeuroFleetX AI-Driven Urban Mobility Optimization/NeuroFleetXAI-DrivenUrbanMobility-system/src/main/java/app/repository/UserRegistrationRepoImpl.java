...
public class UserRegistrationRepoImpl implements UserRegistrationRepo {
    ...
    public UserRegistrationEntity getEntityByEmail(String email) {
        ...
        return(UserRegistrationEntity) entityManagerFactory.createEntityManager().
                createNamedQuery("getEntityByEmail").setParameter("email",email).getSingleResult();
    }
    ...
    @Override
    public boolean updateProfile(UserRegistrationEntity userRegistration) {
        EntityManager entityManager =entityManagerFactory.createEntityManager();
        entityManager.getTransaction().begin();
